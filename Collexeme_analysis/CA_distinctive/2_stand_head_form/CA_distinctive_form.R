...
head.form <- merge(stand_na, stand_iva, by = "stand", all = TRUE)
head(head.form)

# Replace the missing values (NA) with zeros
head.form[is.na(head.form)] <- 0
head(head.form)

# From this point --> Flach's script:

collex.dist(head.form, am = "fye") -> dist.head.form
head(dist.head.form, 12)
tail(dist.head.form, 12)

write.csv(dist.head.form, "YOUR_PATH/distinctive_form.csv", row.names=TRUE)
