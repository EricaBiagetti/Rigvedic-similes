...
head.case <- merge(stand_na, stand_iva, by = "stand", all = TRUE)
head(head.case)

# Replace the missing values (NA) with zeros
head.case[is.na(head.case)] <- 0
head(head.case)

# From this point --> Flach's script:

collex.dist(head.case, am = "fye") -> dist.head.case
head(dist.head.case, 12)
tail(dist.head.case, 12)

write.csv(dist.head.case, "YOUR_PATH/distinctive_case.csv", row.names=TRUE)
