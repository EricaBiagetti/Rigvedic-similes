...
pattern <- merge(stand_na, stand_iva, by = "stand", all = TRUE)
head(pattern)
...
collex.dist(pattern, am = "fye") -> dist.pattern
head(dist.pattern, 12)
tail(dist.pattern, 12)

write.csv(dist.pattern, "YOUR_PATH/distinctive_pattern.csv", row.names=TRUE)
